class History < ApplicationRecord
  ...
  def self.of(history_params, eve_history, today)
    if eve_history.empty?
      return self.new( { :quantity => 0, :median => 0 }.merge(history_params) )
    else
      date_range = one_month(eve_history, today)
      ...
      order_count = sample.map {|hist| hist.order_count }.sum(0.0)
      ...
      history = { quantity: quantity  / (date_range[1] - date_range[0]),
                  median: median_price,
                  order_count: order_count / (date_range[1] - date_range[0]) }.merge(history_params)
      puts history.to_s
      self.new(history)
    end
  end

  def self.one_month(eve_history, latest_date)
    # why did I do that?
